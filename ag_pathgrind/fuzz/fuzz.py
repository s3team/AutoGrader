...
import session
sys.path.append("../ag_pathanalysis/")
from PathAnalysis import pathDeviation
...
class Input:
    def __init__(self, number, filename, bound, submission, bytes=None):
        self.number = number
        ...
        if not bytes:
            ...
            cmd = subprocess.Popen(['./testcase/ag_c1', filename], stderr=subprocess.PIPE, stdout=subprocess.PIPE)
            (stdout1, stderr) = cmd.communicate()
            cmd = subprocess.Popen(['./testcase/ag_' + submission, filename], stderr=subprocess.PIPE, stdout=subprocess.PIPE)
            (stdout2, stderr) = cmd.communicate()   

            if stdout1 != stdout2:
                print("For input " + filename)
                print("Correct Answer should be: " + stdout1)
                print("But the implementation answer is: " + stdout2)
                sys.exit()
            else: 
                args = open(filename, 'r').readline()
                print("Fuzzing Input: " + args + "Output: " + stdout1 + "Running Path Analysis")
                os.chdir("../ag_pathanalysis/")
                args = args.split("\n")[0]
                print(submission)
                print(args)
                pdb.set_trace()
                deviation = pathDeviation("c1", submission, args)
                if deviation:
                    sys.exit()
                os.chdir("../ag_pathgrind/")
...
def expand_execution(input, callbacks, submission):
    '''
	...
	if CONSTRAINT_SUBSUMPTION:
	    ...
	    if CONSTRAINT_SUBSUMPTION:
            ...
            #f.write(stp.pp())
            #f.close()
            new_input = Input(ninput, filename, j + 1, submission, bytes)
...
def search(target, worklist, callbacks, submission):    
    global ninput
    global paths
    global elapsed
    global querytime
    ...
    while worklist:
        ...
        child_inputs = expand_execution(input, callbacks, submission)
...
if __name__ == '__main__':
    ...
    if not worklist:
        ...
        input_seed = Input(0, PARAM['INPUT_FILE'], PARAM.get('MIN_BOUND', 0), submission)
        worklist = [ input_seed ]
    
    search(target, worklist, [ None ] * 10, submission)
