...
class STP:
    ...
    def execute(self):
        ...
        p = subprocess.Popen([ STP_PROG, '-p', '--CVC', '/tmp/stp_input.txt'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def interpret(self):
        ...
        if lines[-2] == 'Valid.':
            ...
            for l in lines[0:-2]:
                l = re.sub('\s*', '', l)
                m = re.match('ASSERT\(x(\d+)=0x([0-9a-zA-Z]+)\)', l)
                if m:
